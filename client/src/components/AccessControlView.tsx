...
export default function AccessControlView() {
  ...
  const [dbTab, setDbTab] = useState<"users" | "leave_requests" | "access_control">("users");
  ...
  const fetchDbRows = async (table: string) => {
    try {
      setDbLoading(true);
      setDbError("");
      const response = await fetch(`/api/users/database-view?table=${table}`);
      if (!response.ok) {
        throw new Error("Failed to fetch database table.");
      }
      const data = await response.json();
      setDbRows(data);
    } catch (err: any) {
      setDbError(err.message || "An error occurred while loading table.");
    } finally {
      setDbLoading(false);
    }
  };
  ...
  return (
    <div className="my-requests-page" style={{ maxWidth: "1000px", margin: "0 auto", padding: "24px 16px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        <div>
          <h2 className="page-title" style={{ margin: 0 }}>System Management</h2>
          <p style={{ fontSize: "0.88rem", color: "var(--cal-ink-muted)", margin: "4px 0 0 0" }}>
            Admin console for team portal access list and live database inspection.
          </p>
        </div>
      </div>

      {/* Navigation Tabs */}
      <div style={{ display: "flex", gap: "10px", marginBottom: "24px", borderBottom: "1px solid var(--cal-border)", paddingBottom: "12px" }}>
        <button
          type="button"
          onClick={() => {
            setActiveTab("access");
          ...
          style={{
            background: activeTab === "access" ? "var(--color-accent)" : "transparent",
            color: activeTab === "access" ? "white" : "var(--cal-ink)",
            border: activeTab === "access" ? "1px solid var(--color-accent)" : "1px solid var(--cal-border)",
            padding: "8px 16px",
        ...
        <button
          ...
          style={{
            background: activeTab === "database" ? "var(--color-accent)" : "transparent",
            color: activeTab === "database" ? "white" : "var(--cal-ink)",
            border: activeTab === "database" ? "1px solid var(--color-accent)" : "1px solid var(--cal-border)",
            padding: "8px 16px",
            borderRadius: "var(--radius-md)",
            fontSize: "0.88rem",
            fontWeight: 600,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              <span style={{ fontSize: "0.88rem", fontWeight: 600, color: "var(--cal-ink)" }}>Select Table:</span>
              {(["users", "leave_requests", "access_control"] as const).map((table) => (
                <button
                  key={table}
                  type="button"
                  onClick={() => {
                  ...
                  style={{
                    background: dbTab === table ? "var(--color-accent)" : "transparent",
                    color: dbTab === table ? "white" : "var(--cal-ink-muted)",
                    border: dbTab === table ? "1px solid var(--color-accent)" : "1px solid var(--cal-border)",
                    padding: "6px 12px",
                    borderRadius: "var(--radius-sm)",
                    fontSize: "0.85rem",
                    fontWeight: 600,
          ...
          <div style={{
            ...
            <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--cal-border)", background: "var(--cal-outside-bg)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h3 style={{ margin: 0, fontSize: "1rem", fontWeight: 700, textTransform: "capitalize" }}>
                {dbTab.replace("_", " ")} ({filteredDbRows.length} rows)
              </h3>
              <button
                type="button"
                className="btn-secondary animate-hover"
                onClick={() => fetchDbRows(dbTab)}
                disabled={dbLoading}
            ...
            {dbLoading ? (
              ...
              <div style={{ padding: "40px", textAlign: "center", color: "var(--cal-ink-muted)" }}>
                {dbSearch ? "No records found matching your search query." : "Table is empty."}
              </div>
            ) : (
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
                  <thead>
                    <tr style={{ borderBottom: "1px solid var(--cal-border)", fontSize: "0.8rem", color: "var(--cal-ink-muted)", textTransform: "uppercase", background: "var(--cal-outside-bg)" }}>
                      {dbColumns.map((col) => (
                        <th key={col} style={{ padding: "12px 16px" }}>{col.replace("_", " ")}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {filteredDbRows.map((row, idx) => (
                      <tr key={row.id ?? idx} style={{ borderBottom: "1px solid var(--cal-border)", fontSize: "0.88rem" }}>
                        {dbColumns.map((col) => {
                          const val = row[col];
                          let renderedVal: React.ReactNode = String(val);

                          if (val === null || val === undefined) {
                            renderedVal = <span style={{ color: "var(--cal-ink-muted)", fontStyle: "italic" }}>null</span>;
                          } else if (col === "status") {
                            renderedVal = (
                              <span style={{
                                display: "inline-block",
                                ...
                                fontWeight: 600,
                                textTransform: "capitalize",
                                background: val === "approved" ? "var(--approved-bg)" : val === "rejected" ? "var(--rejected-bg)" : "var(--pending-bg)",
                                color: val === "approved" ? "var(--approved-text)" : val === "rejected" ? "var(--rejected-text)" : "var(--pending-text)",
                                border: `1px solid ${val === "approved" ? "var(--approved-border)" : val === "rejected" ? "var(--rejected-border)" : "var(--pending-border)"}`
                              }}>
                                {val}
                              </span>
                            );
                            ...
                            renderedVal = (
                              <span style={{
                                ...
                                fontWeight: 600,
                                textTransform: "capitalize",
                                background: val === "admin" ? "var(--approved-bg)" : val === "approver" ? "#e8f0fe" : "var(--cal-outside-bg)",
                                color: val === "admin" ? "var(--approved-text)" : val === "approver" ? "#1a73e8" : "var(--cal-ink-muted)",
                                border: `1px solid ${val === "admin" ? "var(--approved-border)" : val === "approver" ? "#d2e3fc" : "var(--cal-border)"}`
                              }}>
                                {val}
                              </span>
                            );
                          } else if (typeof val === "string" && val.length > 30) {
                            renderedVal = <span title={val}>{val.substring(0, 28)}...</span>;
                          }
